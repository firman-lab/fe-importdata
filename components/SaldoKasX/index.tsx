...
import { addData, addSaldoKasX } from "../../services/import";
...
import { activeStepRec, saldoKasXRec } from "../../store";
...
export default function SaldoKasX(prop : ActiveStepProps) {
  const { steplength } = prop;
  const [items, setItems] = useRecoilState(saldoKasXRec);
  const [activeStep, setActiveStep] = useRecoilState(activeStepRec);

  // const parseNumberCustom = (number_string: any) => {
  //   var new_number = number_string.toString().replaceAll(".", "");
  //   return new_number;
  // }
