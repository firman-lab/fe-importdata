...
import { addAkrualX, addData } from "../../services/import";
...
import { activeStepRec, saldoAkrualXRec } from "../../store";
...
export default function SaldoAkrualX(prop : ActiveStepProps) {
  const { steplength } = prop;
  const [items, setItems] = useRecoilState(saldoAkrualXRec);
  ...
  const load = async () => {
    ...
    if(!arr.length){
      ...
      const response = await addAkrualX(arr);
      if (response.error) {
        alert("error gais");
